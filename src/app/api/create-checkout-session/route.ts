...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create order summary for metadata (keeping within 500 char limit)
    ...
    const itemSummary = items
      .map((item: CartItem) => `${item.name}(${item.quantity})`)
      .join(", ");
    const shortItemSummary =
    ...
    const sessionConfig = {
      ...
      locale: "en" as const, // English locale
      metadata: {
        customerName: customerInfo.name,
        customerPhone: customerInfo.phone,
        totalItems: totalItems.toString(),
        totalAmount: totalAmount.toFixed(2),
        itemSummary: shortItemSummary,
      },
      ...
      shipping_options: [
        {
          shipping_rate_data: {
            ...
            display_name: "Free shipping to Malaysia",
            delivery_estimate: {
              minimum: {
      ...
      // Additional settings to ensure MYR-only checkout
